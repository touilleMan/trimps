...
#define MEMORY_BASE_ADDRESS 0x0
...
static long cpu_nsec_slice = 1000;
...
PyObject *get_memory(PyObject* self __attribute__((unused)), PyObject* args)
{
    ...
    if (address >= MEMORY_BASE_ADDRESS && address - MEMORY_BASE_ADDRESS < MEMORY_SIZE)
    {
        ...
        const uint32_t value = memory[address - MEMORY_BASE_ADDRESS];
...
PyObject *set_memory(PyObject* self __attribute__((unused)), PyObject* args)
{
    ...
    {
        pthread_mutex_lock(&memory_mutex);
        memory[address - MEMORY_BASE_ADDRESS] = value;
        pthread_mutex_unlock(&memory_mutex);
    }
...
static void instruction_I_LW(const uint32_t instruction)
{
    ...
    if (addr >= MEMORY_BASE_ADDRESS && addr - MEMORY_BASE_ADDRESS < MEMORY_SIZE)
    {
        pthread_mutex_lock(&memory_mutex);
        r[rt] = memory[addr - MEMORY_BASE_ADDRESS];
        pthread_mutex_unlock(&memory_mutex);
    }
    else
        r[rt] = 0;
...
static void instruction_I_SW(const uint32_t instruction)
{
    ...
    {
        pthread_mutex_lock(&memory_mutex);
        memory[addr - MEMORY_BASE_ADDRESS] = r[rt];
        pthread_mutex_unlock(&memory_mutex);
    }
}
